    ...
    <div className="mt-24 flex flex-col items-center lg:items-start ps-0 md:ps-36 2xl:ps-['calc((100vw - 1300px) / 2)']" ref={sectionRef}>
        ...
        <div className="flex flex-col lg:flex-row justify-between items-center lg:items-start max-w-[1330px]">
          <div className="w-4/5 lg:w-[55%] text-center lg:text-left flex flex-col items-center lg:items-start">
            <h2 className={`font-[Urdwin]  antialiased text-[#F3F3F6] font-bold text-[3rem] lg:text-[5.25rem] leading-[3rem] lg:leading-[5.3rem] uppercase fade-up-slider`}><span className="text-bg">Color</span><br/>that inspires</h2>
          ...
          <div className="lg:flex flex-row gap-x-4 justify-center self-end hidden me-56">
            ...
            >
              <img src="/images/chevron-left.svg" alt="brick-img" width={12} height={32} />
            </button>
            <button
              className="rounded-full w-12 h-12 bg-[#2D2D2F] flex justify-center items-center fade-up-slider"
              onClick={() => swiperRef.current?.slideNext()}
            >
              <img src="/images/chevron-right.svg" alt="brick-img" width={12} height={32} />
            </button>
          </div>
        </div>
                ...
                <SwiperSlide key={i}>
                  <div className="rounded-3xl p-3 lg:p-6 ps-0 lg:ps-0 flex flex-col gap-y-3 relative">
                    <div className="absolute top-0 right-0 flex flex-row gap-x-3 justify-center items-center m-6 lg:m-12 z-40">
                      ...
                      >
                        <img src="/images/flip-icon.svg" alt="flip-icon-img" width={20} height={19} className="w-4 h-4" />
                      </div>
                    </div>
                    <div className="w-full relative">
                      <img src={card.front} alt="brick-img" width={480} height={525} className="w-full opacity-0" />
                      ...
                      <div className="w-full card absolute h-full top-0 left-0" style={{ perspective: '5000px' }}>
                        <div className="relative card-inner w-full h-full" style={ flipCardIndex === i ? { transform: 'rotateY(180deg)' } : {  }}>
                          <img src={card.front} alt="brick-img" width={480} height={525} className="w-full h-full absolute front-face" />
                          <img src={card.back} alt="brick-img" width={480} height={525} className="w-full h-full absolute back-face" style={{ transform: 'rotateY(180deg)'}} />
                        </div>
