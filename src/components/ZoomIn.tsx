...
const ZoomIn = ({ font }) => {
  ...
  // Use useLayoutEffect to ensure GSAP triggers after layout changes (including image load)
  useLayoutEffect(() => {
    if (!isImageLoaded) return; // Ensure GSAP runs only after the image is loaded

    // Initialize GSAP animation after image load
    const animationContext = gsap.context(() => {
      if (brickRef.current && brickSectionRef.current) {
        // Set initial styles for the brick
        gsap.set(brickRef.current, { scale: 2.5, paddingBottom: "48px", willChange: "transform" });
        ...
        gsap.to(brickRef.current, {
          ...
          scrollTrigger: {
            ...
            start: () => {
              return `top ${(window.innerHeight - brickSectionRef.current.offsetHeight) / 2}`;
            },
            end: () => {
              let height = 0;
              height += brickRef.current.offsetHeight;
  ...
  return (
    <div className="mt-40 lg:pt-24 flex flex-col items-center">
        ...
        <div className="flex flex-col justify-center max-w-[1300px] mt-12 " ref={brickSectionRef} >
          ...
          <img src="/images/yellow-brick.png" alt="brick-img" width={1300} height={356} className="mt-0 pb-48 mx-auto hidden lg:block lg:w-4/5 xl:w-[90%] 2xl:w-[1300px] max-w-none" style={{ transformOrigin: 'top center'}} ref={brickRef}         onLoad={handleImageLoad} // Add an event listener for image load
  />
        </div>
